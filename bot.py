...
# Нормализация телефона к виду +7...
...
async def reg_driver_name(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        "Можете нажать кнопку ниже, чтобы отправить номер автоматически.",
        reply_markup=kb,
    )
    return REG_PHONE


# Шаг 2 — телефон (контакт или текст)
...
async def reg_driver_docs(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if update.message.text and update.message.text.lower() == "готово":
        ...
        if not reg.get("photos"):
            await update.message.reply_text(
                ...
                "Пожалуйста, отправьте хотя бы одно фото документов или авто."
            )
            return REG_DOCS
    ...
    await update.message.reply_text(
        "Отправьте фото документов/авто или нажмите «Готово»."
    )
    return REG_DOCS
...
# Обработка решения админа из группы
